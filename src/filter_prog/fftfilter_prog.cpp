...
*/
#include "fftfilter_prog.hpp"
#include <sys/mman.h>
#include <fcntl.h>

void FiltersProg::fftfilter_prog::init(unsigned long long _samplespertrace, unsigned long long _numtraces){
...
void FiltersProg::fftfilter_prog::applyFilter(shared_ptr<TraceWithData>& tracewd){
...
void FiltersProg::fftfilter_prog::debugPrint(shared_ptr<Trace>& trace, string filename){
...
void FiltersProg::fftfilter_prog::debugPrint(shared_ptr<ComplexTrace>& trace, string filename){
    ofstream debug(filename);
    if (debug.is_open()){
        for (int i=0; i < trace->size(); i++){
...
void FiltersProg::fftfilter_prog::generateWindows(shared_ptr<Trace>& filt, vector<filterParam>& parameters){
...
void FiltersProg::fftfilter_prog::combineFilter(unsigned long pos, TraceValueType windowValue){
...
void FiltersProg::fftfilter_prog::initFilterOutput(){
...
void FiltersProg::fftfilter_prog::writeFilteredTrace(shared_ptr<TraceWithData> tracewd, unsigned int id){
    writeMutex.lock();
...
void* FiltersProg::fftfilter_prog::getFilteredPointer(unsigned int& newsize){
